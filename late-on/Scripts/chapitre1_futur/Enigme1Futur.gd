...
func _ready() -> void:
	...
	if global.enigme1_Wall1 == false:
		...
		ChronoVoice_timer.wait_time = 0.02 * "Repartons du premier paragraphe: <<Prenons une proposition élémentaire: les entreprises existent pour gagner de l'argent, pas pour en perdre. Ce principe devrait être évident pour tout individu doué de raison.>>".length()
		chronoPote.texture = chrono_neutral
		tweenChronoVoice1.tween_property(PopUpText,"text","Repartons du premier paragraphe: " + '\n' + "<<Prenons une proposition élémentaire: les entreprises existent pour gagner de l'argent, pas pour en perdre. Ce principe devrait être évident pour tout individu doué de raison.>>", ChronoVoice_timer.wait_time)
		ChronoVoice_timer.start()
		ChronoVoice_SFX.play()
		await ChronoVoice_timer.timeout
		ChronoVoice_timer.stop()
...
func _process(delta: float) -> void:
	...
	if Move:
		...
		if CollisionWall == Wall1 :
			...
			KeyOk = true
			mecanism_SFX.play()
			ChronoVoice_timer.wait_time = 0.02 * "Bravo! tu te souviens de la première étape 2ème paragraphe: <<	Mais il ne l'était pas tant que ça à la fin des années 1990, quand les pertes les plus lourdes étaient présentées comme un moyen d'investir dans un avenir plus grand et plus radieux.>>".length()
			chronoPote.texture = chrono_happy
			PopUpText.text = ""
			tweenChronoVoice2.tween_property(PopUpText,"text","Bravo! tu te souviens de la première étape" + '\n' + "2ème paragraphe: <<	Mais il ne l'était pas tant que ça à la fin des années 1990, quand les pertes les plus lourdes étaient présentées comme un moyen d'investir dans un avenir plus grand et plus radieux.>>", ChronoVoice_timer.wait_time)
			ChronoVoice_timer.start()
			ChronoVoice_SFX.play()
			await ChronoVoice_timer.timeout
			ChronoVoice_timer.stop()
			...
			KeyOk = true
			mecanism_SFX.play()
			ChronoVoice_timer.wait_time = 0.02 * "Tu t'en sors vraiment bien! continuons avec le 3ème paragraphe:  Selon la doxa de la <<Nouvelle Economie>>, le nombre de pages consultées sur Internet était considéré comme un indicateur financier plus fiable et plus prospectif qu'un critère aussi prosaïque que le bénéfice net.>>".length()
			chronoPote.texture = chrono_happy
			PopUpText.text = ""
			tweenChronoVoice2.tween_property(PopUpText,"text","Tu t'en sors vraiment bien! continuons avec le 3ème paragraphe: " + '\n' + "<<	Selon la doxa de la <<Nouvelle Economie>>, le nombre de pages consultées sur Internet était considéré comme un indicateur financier plus fiable et plus prospectif qu'un critère aussi prosaïque que le bénéfice net.>>", ChronoVoice_timer.wait_time)
			ChronoVoice_timer.start()
			ChronoVoice_SFX.play()
			await ChronoVoice_timer.timeout
			ChronoVoice_timer.stop()
			...
			KeyOk = true
			mecanism_SFX.play()
			ChronoVoice_timer.wait_time = 0.02 * "Tu es arrivé si loin! est-ce que tu trouveras le 5° paragraphe aussi? Mais les distorsions qu'entraînent les bulles ne disparaissent pas dès qu'elles éclatent. La folie Internet des années 1990 était la plus grosse bulle depuis le krach de 1929 et les leçons qu'on a retenues définissent et altèrent presque toute la vision technologique actuelle.>>".length()
			chronoPote.texture = chrono_happy
			PopUpText.text = ""
			tweenChronoVoice2.tween_property(PopUpText,"text","Tu es arrivé si loin! est-ce que tu trouveras le 5° paragraphe aussi? " + '\n' + "<<	Mais les distorsions qu'entraînent les bulles ne disparaissent pas dès qu'elles éclatent. La folie Internet des années 1990 était la plus grosse bulle depuis le krach de 1929 et les leçons qu'on a retenues définissent et altèrent presque toute la vision technologique actuelle.>>", ChronoVoice_timer.wait_time)
			ChronoVoice_timer.start()
			ChronoVoice_SFX.play()
			await ChronoVoice_timer.timeout
			ChronoVoice_timer.stop()
			...
			tweenWall.tween_property(WallOff2,"modulate:a", 1,0.6)
			ChronoVoice_timer.wait_time = 0.02 * "C'est la bonne combinaison! Ton compte est ouvert! Félicitations petit employé :)".length()
			chronoPote.texture = chrono_success
			PopUpText.text = ""
			tweenChronoVoice2.tween_property(PopUpText,"text","C'est la bonne combinaison! Ton compte est ouvert! Félicitations petit employé :)", ChronoVoice_timer.wait_time)
			ChronoVoice_timer.start()
			ChronoVoice_SFX.play()
			await ChronoVoice_timer.timeout
			ChronoVoice_timer.stop()
			...
			if global.enigme1f_Nbtry == 1:
				Wall1.modulate = Color(0, 0.5, 1)
			if global.enigme1f_Nbtry == 2:
				Wall2.modulate = Color(0, 0.5, 1)
			if global.enigme1f_Nbtry == 3:
				Wall3.modulate = Color(0, 0.5, 1)
			if global.enigme1f_Nbtry == 4:
				Wall4.modulate = Color(0, 0.5, 1)
			if global.enigme1f_Nbtry == 5:
				Wall5.modulate = Color(0, 0.5, 1)
			tweenMove.kill()
			tweenBounce.kill()
			tweenWall.kill()
			var tweenBall1 = get_tree().create_tween() 
			tweenBall1.tween_property(Ball,"modulate:a", 0,0.3)
			Ball.global_position = initialPos
			error_SFX.play()
			ChronoVoice_timer.wait_time = 0.02 * "Oh non, ce n'est pas le bon mot. Mais ne t'en fais pas, tu vas y arriver ^^".length()
			chronoPote.texture = chrono_error
			PopUpText.text = ""
			tweenChronoVoice2.tween_property(PopUpText,"text","Oh non, ce n'est pas le bon mot. Mais ne t'en fais pas, tu vas y arriver ^^", ChronoVoice_timer.wait_time)
			ChronoVoice_timer.start()
			ChronoVoice_SFX.play()
			await ChronoVoice_timer.timeout
			ChronoVoice_timer.stop()
			...
			tweenBall1.kill()
			var tweenBall2 = get_tree().create_tween() 
			var tweenWall2 = get_tree().create_tween()
			...
			tweenWall2.tween_property(Wall1,"modulate:a", 1,0.2)
			tweenWall2.tween_property(WallOff1,"modulate:a", 1,0.2)
			tweenWall2.tween_property(WallOff2,"modulate:a", 0,0.2)
			tweenWall2.tween_property(WallOff3,"modulate:a", 0,0.2)
			tweenWall2.tween_property(WallOff4,"modulate:a", 1,0.2)
			tweenBall2.tween_property(Ball,"modulate:a", 1,0.3)
			KeyOk = true
